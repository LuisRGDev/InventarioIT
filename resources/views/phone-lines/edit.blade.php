        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    {{ __('Editar Línea Telefónica:') }} {{ $phoneLine->number }}
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                <form action="{{ route('phone-lines.update', $phoneLine) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                                <div>
                                    ...
                                    <x-text-input id="number" name="number" type="text" class="mt-1 block w-full font-mono"
                                                  value="{{ old('number', $phoneLine->number) }}" required placeholder="Ej. 55 1234 5678" />
                                    <x-input-error :messages="$errors->get('number')" class="mt-1" />
                                </div>
                                <div>
                                    <x-input-label for="status" value="Estatus Actual *" />
                                    <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        @foreach(App\Enums\PhoneLineStatus::cases() as $status)
                                            <option value="{{ $status->value }}" {{ old('status', $phoneLine->status->value) == $status->value ? 'selected' : '' }}>
                                                {{ $status->label() }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="data_plan" name="data_plan" type="text" class="mt-1 block w-full"
                                                  value="{{ old('data_plan', $phoneLine->data_plan) }}" placeholder="Ej. Plan Telcel Max 1000" />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="plan_cost" name="plan_cost" type="number" step="0.01" class="mt-1 block w-full"
                                                  value="{{ old('plan_cost', $phoneLine->plan_cost) }}" placeholder="Ej. 499.00" />
                        ...
                        <div>
                            ...
                            <div>
                                <textarea id="notes" name="notes" rows="3"
                                          ...
                                          placeholder="Añade observaciones generales sobre la línea...">{{ old('notes', $phoneLine->notes) }}</textarea>
                    ...
                    <div class="px-6 py-4 bg-gray-50 flex items-center justify-end border-t border-gray-100">
                        <a href="{{ route('phone-lines.show', $phoneLine) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150 mr-3">
                            Cancelar
                        </a>
