<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4" x-data>
                <button type="button" @click.stop="$dispatch('open-phone-line-import-modal')" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('phone-lines.show', $line) }}" class="text-indigo-600 hover:text-indigo-900 p-2.5 rounded-lg hover:bg-indigo-50 transition" title="Ver detalles" aria-label="Ver detalles">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <form action="{{ route('phone-lines.destroy', $line) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar esta línea telefónica?')" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-gray-400 hover:text-red-600 p-2.5 rounded-lg hover:bg-red-50 transition" title="Eliminar" aria-label="Eliminar">
    ...
    <x-import-modal name="phone-line-import" title="Importar Líneas Telefónicas"
                     :action="route('phone-lines.import')"
                     :import-route="route('phone-lines.import.template')">
