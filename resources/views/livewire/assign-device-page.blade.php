    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-gray-800">Nueva Asignación</h2>
                    <p class="text-sm text-gray-500">Selecciona un empleado y el equipo que deseas asignarle.</p>
                </div>
            </div>
            ...
            <div class="p-6">
                    ...
                    <div>
                        ...
                        @if ($this->selectedDevice)
                            <div class="p-4 rounded-lg border border-emerald-200 bg-emerald-50/50 flex items-center justify-between">
                                <div>
                                    <p class="font-medium text-emerald-900">{{ $this->selectedDevice->brand }} {{ $this->selectedDevice->model }}</p>
                                    <p class="text-sm text-emerald-700 font-mono">SN: {{ $this->selectedDevice->serial_number }}</p>
                                </div>
                                <button type="button" wire:click="clearDevice" class="text-emerald-600 hover:text-emerald-800 p-2 rounded-full hover:bg-emerald-100 transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                </button>
                            </div>
                        @else
                            <div class="relative">
                                <input type="text" wire:model.live.debounce.300ms="deviceSearch" placeholder="Buscar por serial, marca o modelo..." class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 pl-10 text-sm">
                                <div class="absolute left-3 top-2.5 text-gray-400">
                            ...
                            @error('selectedDeviceId') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror

                            @if(strlen($deviceSearch) > 0)
                                <div class="mt-2 border border-gray-200 rounded-lg shadow-sm bg-white overflow-hidden max-h-60 overflow-y-auto">
                                    @forelse($this->availableDevices as $dev)
                                        <button type="button" wire:click="selectDevice({{ $dev->id }})" class="w-full text-left px-4 py-3 hover:bg-gray-50 border-b border-gray-100 last:border-0 transition">
                                            <p class="text-sm font-medium text-gray-900">{{ $dev->brand }} {{ $dev->model }}</p>
                                            <p class="text-xs text-gray-500 font-mono">SN: {{ $dev->serial_number }}</p>
                                        </button>
                                    @empty
                                        <div class="px-4 py-3 text-sm text-gray-500 text-center">No hay equipos disponibles que coincidan.</div>
                                    @endforelse
                                </div>
                            @endif
                        @endif
                ...
                <div class="mt-8 border-t border-gray-100 pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <x-input-label for="conditionOnDelivery" value="Condición de Entrega *"/>
                            <select wire:model="conditionOnDelivery" id="conditionOnDelivery" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                @foreach($this->conditions as $cond)
                                    <option value="{{ $cond->value }}">{{ $cond->label() }}</option>
                                @endforeach
                            </select>
                            @error('conditionOnDelivery') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        <div>
                ...
                @if($showConfirm)
                    <div class="mt-8 p-5 bg-orange-50 border border-orange-200 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-orange-700 mt-1">El equipo pasará al estatus "Asignado" y se enviará un correo de notificación.</p>
                        </div>
