        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <h2 class="font-black text-2xl text-slate-900 leading-tight tracking-tight flex items-center gap-2.5">
                    <span class="w-2.5 h-7 bg-gradient-to-b from-blue-500 to-indigo-500 rounded-full inline-block shadow-sm"></span>
                    {{ __('Editar Puesto') }}: <span class="text-middleby-700">{{ $jobPosition->name }}</span>
                </h2>
                <p class="text-xs sm:text-sm font-semibold text-slate-500 mt-1">Actualiza la Dirección, Área y Puesto.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
                <form action="{{ route('job-positions.update', $jobPosition) }}" method="POST" class="p-6 sm:p-10 space-y-8">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="direction" name="direction" type="text" class="mt-2 block w-full bg-slate-50/50 focus:bg-white" :value="old('direction', $jobPosition->direction)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="area" name="area" type="text" class="mt-2 block w-full bg-slate-50/50 focus:bg-white" :value="old('area', $jobPosition->area)" required />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-2 block w-full bg-slate-50/50 focus:bg-white text-lg font-bold" :value="old('name', $jobPosition->name)" required />
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="3" class="mt-2 block w-full border-slate-200 focus:border-middleby-500 focus:ring-middleby-500 rounded-xl shadow-sm text-sm font-medium bg-slate-50/50 focus:bg-white transition">{{ old('notes', $jobPosition->notes) }}</textarea>
