    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">{{ $employee->name }}
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center flex-shrink-0">
                        <span class="text-indigo-700 font-bold text-base">{{ $initials }}</span>
                    </div>
                    <div>
                        <p class="font-semibold text-gray-900">{{ $employee->name }}</p>
                        <p class="text-xs text-gray-400 mt-0.5">{{ $employee->position }} — {{ $employee->department }}</p>
                    </div>
                </div>
                ...
                <div class="h-8 w-px bg-gray-200 hidden sm:block"></div>
                <div class="text-center">
                    <p class="text-2xl font-bold text-indigo-600">{{ $assignments->where('returned_at', null)->count() }}</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Cond. entrega</th>
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Cond. retorno</th>
                            ...
                            @forelse($assignments as $assignment)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        ...
                                        @else
                                            <span class="text-gray-400 text-sm italic">Equipo eliminado</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-600">
                                        {{ $assignment->device?->category?->name ?? '—' }}
                                    </td>
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3 text-gray-400">
                                            ...
                                            <p class="text-sm font-medium">Este empleado no tiene historial de asignaciones</p>
                                            <a href="{{ route('assignments.assign') }}"
                                               class="text-sm text-indigo-600 hover:underline">Asignar primer equipo</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
